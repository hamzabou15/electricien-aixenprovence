    ...
    <>
      ...
      <header className=" right-0 bg-white/95 backdrop-blur-md shadow-md z-100 sticky top-0">
        <div className="max-w-[1300px] mx-auto flex items-center justify-between px-6 md:px-8 py-4 ">
          ...
          {/* Menu desktop */}
          <nav className="hidden lg:flex items-center gap-10 text-[#003049] font-semibold tracking-wide">
            <Link
              href="/"
              className="relative group py-2"
            >
              Accueil
              <span className="absolute left-0 bottom-0 w-0 h-[3px] bg-[#FFD600] transition-[width] group-hover:w-full"></span>
            </Link>
            ...
            <div className="relative group">
              ...
              >
                Services
                <svg
                  className="w-4 h-4 mt-[2px] stroke-[#003049] group-hover:stroke-[#FFD600] transition-colors"
                  fill="none"
                  strokeWidth={2}
                  viewBox="0 0 24 24"
                ...
                {servicesData.map((service, idx) => (
                  <li key={idx}>
                    <Link
                      ...
                      className="block px-5 py-2 hover:bg-[#FFD600] hover:text-[#003049] transition-colors rounded-md"
                    >
                      {service.title}
                    </Link>
                  </li>
          ...
          <div className="hidden lg:flex items-center gap-5">
            <a
              ...
              className="flex items-center gap-2 text-sm font-semibold text-[#003049] hover:text-[#c1121f] transition-colors"
            >
              <MdLocalPhone className="w-8 h-8 p-2 rounded-full bg-[#FFD600] text-[#003049]" />
              +33 7 56 93 52 00
            </a>
          ...
          <button
            ...
            className="lg:hidden text-3xl text-[#003049] focus:outline-none focus:ring-2 focus:ring-[#FFD600] rounded"
          >
      ...
      >
        <nav className="flex flex-col px-6 py-8 gap-6 text-[#003049] font-semibold">
          <Link
            href="/"
          ...
          >
            Services
            <svg
              className={`w-5 h-5 stroke-[#003049] transition-transform duration-300 ${
                mobileServicesOpen ? "rotate-180 stroke-[#FFD600]" : ""
              }`}
              fill="none"
              strokeWidth={2}
              viewBox="0 0 24 24"
          ...
          {mobileServicesOpen && (
            <ul
              ...
              className="pl-4 border-l-4 border-[#FFD600] flex flex-col gap-2"
            >
              {servicesData.map((service, idx) => (
                <li key={idx}>
                  <Link
                    href={`/services/${service.slug}`}
                    className="block text-[#0055AA] hover:text-[#c1121f] transition-colors"
                    onClick={() => setMobileOpen(false)}
                  >
                    {service.title}
                  </Link>
          ...
          <a
            ...
            onClick={() => setMobileOpen(false)}
          >
            <MdLocalPhone className="w-10 h-10 p-2 bg-[#FFD600] rounded-full text-[#003049]" />
            +33 7 56 93 52 00
          </a>
